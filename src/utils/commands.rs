...
pub fn run_command(
    ...
    let status: ExitStatus = if sudo {
        Command::new("sudo").args(&args).status().expect(&format!(
            "{}",
            colors::output_background_color("red", failure_message_1)
        ))
    } else {
        Command::new(&args[0])
            .args(&args[1..])
    ...
    if status.success() {
        println!(
            "{}",
            colors::output_background_color("green", success_message_1)
        );
    } else {
        println!(
            "{}",
            colors::output_background_color("red", failure_message_2)
        );
    }
